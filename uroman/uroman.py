...
def slot_value_in_double_colon_del_list(
    line: str, slot: str, default: str | list | None = None
) -> str | list | None:
    ...
    m = regex.match(rf"(?:.*\s)?::{slot}(|\s+\S.*?)(?:\s+::\S.*|\s*)$", line)
...
class Uroman:
    ...
    def __init__(
        self, data_dir: Path | None = None, **args
    ):  # args: load_log, rebuild_ud_props
        ...
        self.dict_num = defaultdict(
            lambda: None
        )  # values are int (most common), float, or str ("1/2")
        # num_props key: txt
        ...
        self.abugida_cache = (
            {}
        )  # key: (script, char_rom) value: (base_rom, base_rom_plus_abugida_vowel, modified rom)
        self.load_resource_files(
            self.data_dir,
            args.get("load_log", False),
    ...
    def second_rom_filter(
        self, c: str, rom: str, name: str | None
    ) -> Tuple[str | None, str]:
    ...
    def load_rom_file(
        ...
        with f:
            for line_number, line in enumerate(f, 1):
                ...
                if file_format == "u2r":
                    ...
                    if tone_mark := slot_value_in_double_colon_del_list(
                        line, "tone-mark"
                    ):
                        self.dict_str[("tone-mark", s)] = tone_mark
                    if syllable_info := slot_value_in_double_colon_del_list(
                        line, "syllable-info"
                    ):
                    ...
                    t_at_end_of_syllable = dequote_string(
                ...
                is_minus_sign = has_value_in_double_colon_del_list(
                    line, "is-minus-sign"
                )
                if is_minus_sign:
                ...
                is_decimal_point = has_value_in_double_colon_del_list(
                    line, "is-decimal-point"
                )
                is_large_power = has_value_in_double_colon_del_list(
                    line, "is-large-power"
                )
                fraction_connector = slot_value_in_double_colon_del_list(
                    line, "fraction-connector"
                )
                if fraction_connector:
                    self.fraction_connectors[s] = True
                percentage_marker = slot_value_in_double_colon_del_list(
                    line, "percentage-marker"
                )
                int_frac_connector = slot_value_in_double_colon_del_list(
                    line, "int-frac-connector"
                )
                lcode_s = slot_value_in_double_colon_del_list(line, "lcode")
                lcodes = regex.split(r"[,;]\s*", lcode_s) if lcode_s else []
                use_only_at_start_of_word = has_value_in_double_colon_del_list(
                    line, "use-only-at-start-of-word"
                )
                dont_use_at_start_of_word = has_value_in_double_colon_del_list(
                    line, "dont-use-at-start-of-word"
                )
                use_only_at_end_of_word = has_value_in_double_colon_del_list(
                    line, "use-only-at-end-of-word"
                )
                dont_use_at_end_of_word = has_value_in_double_colon_del_list(
                    line, "dont-use-at-end-of-word"
                )
                use_only_for_whole_word = has_value_in_double_colon_del_list(
                    line, "use-only-for-whole-word"
                )
                num_s = slot_value_in_double_colon_del_list(line, "num")
                ...
                if s is not None:
                    ...
                    if any_not_none(
                        ...
                        if (
                            (len(old_rom_rules) == 1)
                            and (old_rom_rules[0]["prov"] in ("ud", "ow"))
                            and not (
                                lcodes
                                or use_only_at_start_of_word
                                or dont_use_at_start_of_word
                                or use_only_at_end_of_word
                                or dont_use_at_end_of_word
        ...
        thai_vowel_modifiers = ["\u0E31", "\u0E47"] + list(
            map(chr, range(0x0E33, 0x0E3B))
        )
    ...
    def load_script_file(self, filename: str, load_log: bool = True):
        ...
        with f:
            for line_number, line in enumerate(f, 1):
                ...
                if regex.match(r"^\s*$", line):  # blank line
                    continue
                line = regex.sub(r"\s{2,}#.*$", "", line)
                if script_name := slot_value_in_double_colon_del_list(
                    line, "script-name"
                ):
                    lc_script_name = script_name.lower()
                    if lc_script_name in self.scripts:
                        sys.stderr.write(
                            f'** Ignoring duplicate script "{script_name}" '
                            f"in line {line_number} of {filename}\n"
                        )
                    else:
                        n_entries += 1
                        direction = slot_value_in_double_colon_del_list(
                            line, "direction"
                        )
                        abugida_default_vowel_s = slot_value_in_double_colon_del_list(
                            line, "abugida-default-vowel"
                        )
                        abugida_default_vowels = (
                            regex.split(r"[,;]\s*", abugida_default_vowel_s)
                            if abugida_default_vowel_s
                            else []
                        )
                        alt_script_name_s = slot_value_in_double_colon_del_list(
                            line, "alt-script-name"
                        )
                        alt_script_names = (
                            regex.split(r"[,;]\s*", alt_script_name_s)
                            if alt_script_name_s
                            else []
                        )
                        language_s = slot_value_in_double_colon_del_list(
                            line, "language"
                        )
                        languages = (
                            regex.split(r"[,;]\s*", language_s) if language_s else []
                        )
                        new_script = Script(
                            script_name=script_name,
                            alt_script_names=alt_script_names,
                            languages=languages,
                            direction=direction,
                        ...
                        for alt_script_name in alt_script_names:
                            ...
                            if lc_alt_script_name in self.scripts:
                                sys.stderr.write(
                                    f'** Ignoring duplicate alternative script name "{script_name}" '
                                    f"in line {line_number} of {filename}\n"
                                )
                            else:
        ...
        if load_log:
            sys.stderr.write(
                f"Loaded {n_entries} script descriptions from {filename}"
                f" (max_n_scripts_name_components: {max_n_script_name_components})\n"
            )

    def extract_script_name(
        self, script_name_plus: str, full_char_name: str = None
    ) -> str | None:
    ...
    def load_unicode_data_props(self, filename: str, load_log: bool = True):
        ...
        (
            ...
            n_script_char,
            n_script_vowel_sign,
            n_script_medial_consonant_sign,
            n_script_virama,
        ) = (0, 0, 0, 0, 0)
        try:
            f = open(filename, "r", encoding="utf-8")
            ...
            for line_number, line in enumerate(f, 1):
                ...
                if script_name := slot_value_in_double_colon_del_list(
                    ...
                    for char in slot_value_in_double_colon_del_list(line, "char", []):
                        ...
                        n_script_char += 1
                    for char in slot_value_in_double_colon_del_list(
                        line, "numeral", []
                    ):
                        self.dict_str[("script", char)] = script_name
                        n_script_char += 1
            ...
            for line_number, line in enumerate(f, 1):
                ...
                if isinstance(d, dict):
                    if txt := d.get("txt"):
                        ...
                        sys.stderr.write(
                            f"Missing txt in l.{line_number} in file {filename}: {line.strip()}\n"
                        )
                    ...
                    sys.stderr.write(
                        f"json in l.{line_number} in file {filename} not a dict: {line.strip()}\n"
                    )
        if load_log:
        ...
        for char in s:
            if decomp := ud.decomposition(char).split():
                try:
                    ...
                    letters = [
                        x for x in decomp_chars if ud.category(x).startswith("L")
                    ]
                except ValueError:
        ...
        with f:
            for line_number, line in enumerate(f, 1):
                ...
                try:
                    ...
                    sys.stderr.write(
                        f"Cannot process line {line_number} in file {filename}: {line}"
                    )
                else:
                    s = chinese
    ...
    @staticmethod
    def add_char_to_rebuild_unicode_data_dict(
        d: dict, script_name: str, prop_class: str, char: str
    ):
    ...
    def rebuild_unicode_data_props(
        self, out_filename: str, cjk: str = None, hangul: str = None
    ):
        ...
        while codepoint < 0xF0000:
            ...
            if not (char_name := self.chr_name(c)):
                continue
            # noinspection SpellCheckingInspection
            for prop_name_comp2 in (
                "VOWEL SIGN",
                (
                    "MEDIAL CONSONANT SIGN",
                    "CONSONANT SIGN MEDIAL",
                    "CONSONANT SIGN SHAN MEDIAL",
                ...
                for prop_name_comp in prop_list:
                    ...
                    script_name_cand = regex.sub(
                        rf"\s+{prop_name_comp}\b.*$", "", char_name
                    )
                    if script_name := self.extract_script_name(
                        script_name_cand, char_name
                    ):
                        self.add_char_to_rebuild_unicode_data_dict(
                            d, script_name, prop_class, c
                        )
            script_name_cand = regex.sub(
                r"\s+(CONSONANT|LETTER|LIGATURE|SIGN|SYLLABLE|SYLLABICS|VOWEL|"
                        ...
                        if key in d:
                            if chars := "".join(d[key]):
                                if prop_class in ("char",):
                                    prop_components.append(
                                        f"::n-{prop_class} {len(chars)}"
                                    )
        ...
        sys.stderr.write(
            f"Rebuilt {out_filenames} with {n_script_refs} characters "
    ...
    def rebuild_num_props(self, out_filename: str, err_filename: str):
        ...
        with (
            ...
            while codepoint < 0xF0000:
                ...
                char = chr(codepoint)
                num = first_non_none(
                    ud_numeric(char), self.num_value(char)  # robust ud.numeric
                )  # uroman table includes extra num values, e.g. for Egyptian
                if num is None:
                    continue
                result_dict = {}
                orig_txt = char
                ...
                elif ("FRACTION" in name) and (
                    fraction := fraction_char2fraction(char, num, self)
                ):
                    fraction = fraction
                    num_type = "fraction"
                else:
                ...
                fraction_list = (
                    None
                    if fraction is None
                    else [fraction.numerator, fraction.denominator]
                )
    ...
    def load_resource_files(
        ...
        data_dir: Path,
        load_log: bool = False,
        rebuild_ud_props: bool = False,
        rebuild_num_props: bool = False,
    ):
        """Loads all resource files needed for romanization."""
        data_dir = data_dir
        if not isinstance(data_dir, Path):
            sys.stderr.write(
                f"Error: data_dir is of {type(data_dir)}, not a Path.\n"
                f"       Cannot load any resource files.\n"
            )
            return
        self.load_rom_file(
            os.path.join(data_dir, "romanization-auto-table.txt"),
            "ud",
            file_format="rom",
            load_log=load_log,
        )
        self.load_rom_file(
            os.path.join(data_dir, "UnicodeDataOverwrite.txt"),
            "ow",
            file_format="u2r",
            load_log=load_log,
        )
        self.load_rom_file(
            os.path.join(data_dir, "romanization-table.txt"),
            "man",
            file_format="rom",
            load_log=load_log,
        )
        ...
        for base_file in (
            "UnicodeDataProps.txt",
            "UnicodeDataPropsCJK.txt",
            "UnicodeDataPropsHangul.txt",
        ):
            self.load_unicode_data_props(
                os.path.join(data_dir, base_file), load_log=load_log
            )
        if rebuild_ud_props:
            self.rebuild_unicode_data_props(
                os.path.join(data_dir, "UnicodeDataProps.txt"),
        ...
        if rebuild_num_props:
            self.rebuild_num_props(
                os.path.join(data_dir, "NumProps.jsonl"),
    ...
    def unicode_float2fraction(
        self, num: float, precision: float = 0.000001
    ) -> Tuple[int, int] | None:
    ...
    def test_output_of_selected_scripts_and_rom_rules(self):
        ...
        for offset, c in enumerate(
            f"9九万萬百፲፱፻፸¾0²₂AⅫ⑫൵{runic90}{mayan12}{egyptian600}{klingon2}"
        ):
    ...
    def romanize_file(
        ...
        **args,
        ...
        if direct_input and (input_filename is None):
            ...
            try:
                f_in = open(
                    input_filename, "r", encoding="utf-8", errors="surrogateescape"
                )
                f_in_to_be_closed = True
            except OSError:
                sys.stderr.write(
                    f"Error in romanize_file: Cannot open file {input_filename}\n"
                )
                f_in = None
            except UnicodeDecodeError:
                sys.stderr.write(
                    f"Error in romanize_file: File not in UTF-8: {input_filename}\n"
                )
                f_in = None
            ...
            sys.stderr.write(
                f"Error in romanize_file: argument 'input_filename' {input_filename} "
        ...
        if isinstance(output_filename, str):
            try:
                ...
                sys.stderr.write(
                    f"Error in romanize_file: Cannot write to file {output_filename}\n"
                )
                f_out = None
            ...
            sys.stderr.write(
                f"Error in romanize_file: argument 'output_filename' {output_filename} "
        ...
        if f_in and f_out:
            ...
            try:
                for line_number, line in enumerate(f_in, 1):
                    if non_utf8_chars := regex.findall(r"[\uDC80-\uDCFF]", line):
                        ...
                        if self.n_error_messages_output < max_n_error_messages:
                            ...
                            sys.stderr.write(
                                f"Detected encoding error: file {input_filename} line {line_number} "
                            ...
                            sys.stderr.write(
                                f"Too many errors. No further errors reported.\n"
                            )
                            self.n_error_messages_output += 1
                        line = line2
                    if m := regex.match(r"(::lcode\s+)([a-z]{3})(\s+)(.*)$", line):
                        ...
                        if args.get("rom_format", RomFormat.STR) == RomFormat.STR:
                            ...
                            prefixed_edges = [lcode_prefix] + self.romanize_string(
                                snt, lcode2 or lcode, **args
                            )
                            f_out.write(Edge.json_str(prefixed_edges) + "\n")
                    else:
                        f_out.write(
                            Edge.json_str(
                                self.romanize_string(line.rstrip("\n"), lcode, **args)
                            )
                            + "\n"
                        )
                ...
                sys.stderr.write(
                    f"   Please make sure the input stream is in Unicode (UTF-8).\n"
                )
                sys.stderr.write(
                    f"   Consider setting the encoding to UTF-8,"
                    f" e.g. set/export PYTHONIOENCODING=UTF-8\n"
                )
                sys.stderr.write(
                    f"   Consider using uroman with -i/--input_filename option, "
        ...
        if input_filename and self.n_non_utf8_characters:
            sys.stderr.write(
                f"Total number of non-UTF-8 characters in {input_filename}: "
                f"{self.n_non_utf8_characters}\n"
            )

    @staticmethod
    def apply_any_offset_to_cached_rom_result(
        cached_rom_result: str | List[Edge], offset: int = 0
    ) -> str | List[Edge]:
        if isinstance(cached_rom_result, str):
            ...
            return [
                Edge(edge.start + offset, edge.end + offset, edge.txt, edge.type)
                for edge in cached_rom_result
            ]
    ...
    def romanize_string_core(
        self, s: str, lcode: str | None, rom_format: RomFormat, offset: int = 0, **args
    ) -> str | List[Edge]:
    ...
    def romanize_string(
        ...
        s: str,
        lcode: str | None = None,
        rom_format: RomFormat = RomFormat.STR,
        **args,
    ) -> str | List[Edge]:
        ...
        if self.cache_p:
            ...
            while m3 := regex.match(r"(.*?)([.,; ]*[ 。་][.,; ]*)(.*)$", rest):
                ...
                result += self.romanize_string_core(
                    pre, lcode, rom_format, offset, **args
                )
                offset += len(pre)
                result += self.romanize_string_core(
                    delimiter, lcode, rom_format, offset, **args
                )
...
class NumEdge(Edge):
    def __init__(
        self, start: int, end: int, s: str, uroman: Uroman | None, active: bool = False
    ):
        ...
        self.value, self.fraction, self.num_base, self.base_multiplier = (
            None,
            None,
            None,
            None,
        )
        self.type, self.script, self.is_large_power, self.active = (
            None,
            None,
            False,
            active,
        ...
        if start + 1 == end:
            ...
            if d := uroman.num_props.get(char):
                ...
                self.fraction = (
                    Fraction(fraction_list[0], fraction_list[1])
                    if fraction_list
                    else None
                )
    ...
    def update(
        ...
        fraction_s = (
            ...
            else f"{self.fraction.numerator}/{self.fraction.denominator}"
        )
        delimiter_s = " " if value_s and fraction_s else ""
    ...
    def __str__(self):
        ...
        return (
            ...
            + (
                f" V:{self.value}"
                if ((self.value is not None) and (str(self.value) != self.txt))
                else ""
            )
            + (
                f" VS:{self.value_s}"
                if ((self.value_s is not None) and (self.value_s != self.txt))
                else ""
            )
...
class Lattice:
    ...
    def is_at_start_of_word(self, position: int) -> bool:
        ...
        if (preceded_by_alpha := self.props.get(("preceded_by_alpha", end), None)) in (
            True,
            False,
        ):
        ...
        for start in self.lattice[(end, "left")]:
            for edge in self.lattice[(start, end)]:
                ...
                if len(edge.txt) and (
                    prev_letter.isalpha()
                    or (first_char_is_braille and (prev_letter in ["'"]))
                ):
                    self.props[("preceded_by_alpha", position)] = True
                    return False
    ...
    def is_at_end_of_word(self, position: int) -> bool:
        if (
            cached_followed_by_alpha := self.props.get(
                ("followed_by_alpha", position), None
            )
        ) in (True, False):
        ...
        for end in range(start + 1, self.max_vertex + 1):
            ...
            for rom_rule in self.uroman.rom_rules[s]:
                rom = rom_rule["t"]
                if (not rom_rule["use-only-at-start-of-word"]) and regex.search(
                    r"\pL", rom
                ):
                    self.props[("followed_by_alpha", position)] = True
                    return False
    ...
    def is_at_end_of_syllable(self, position: int) -> Tuple[bool, str]:
        ...
        next_char2 = (
            self.s[adj_position + 1] if adj_position + 1 < self.max_vertex else None
        )
        ...
        if (
            self.uroman.dict_str[("syllable-info", prev_char)]
            == "written-pre-consonant-spoken-post-consonant"
        ):
            return False, "pre-post-vowel-on-left"
        if (
            self.uroman.dict_str[("syllable-info", next_char)]
            == "written-pre-consonant-spoken-post-consonant"
        ):
        ...
        next_char_rom = first_non_none(
            self.simple_top_romanization_candidate_for_span(
                adj_position, adj_position + 2, simple_search=True
            ),
            self.simple_top_romanization_candidate_for_span(
                adj_position, adj_position + 1, simple_search=True
            ),
            "?",
        )
        ...
        if (next_char == "\u0E2D") and (next_char2 is not None):  # THAI CHARACTER O ANG
            next_char2_rom = first_non_none(
                self.simple_top_romanization_candidate_for_span(
                    adj_position + 1, adj_position + 2, simple_search=True
                ),
                "?",
            )
            if regex.match(r"[aeiou]", next_char2_rom.lower()):
                return (
                    True,
                    "o-ang-followed-by-vowel",
                )  # In that context Thai char. "o ang" is considered a consonant
    ...
    def expand_rom_with_special_chars(
        self, rom: str, start: int, end: int, **args
        ...
        if (
            (prev_char and prev_char in "っッ\u0A71")
        ...
        if uroman.chr_script_name(first_char) == "Thai":
            if (start + 1 == end) and regex.match(r"[bcdfghjklmnpqrstvwxyz]+$", rom):
                if (
                    uroman.dict_str[("syllable-info", prev_char)]
                    == "written-pre-consonant-spoken-post-consonant"
                ):
                        ...
                        if vowel_prefix_len <= start:
                            for vowel_suffix_len in [3, 2, 1]:
                                if end + vowel_suffix_len <= len(full_string):
                                    pattern = (
                                        full_string[start - vowel_prefix_len : start]
                                        + "–"
            ...
            if (
                (uroman.chr_script_name(prev_char) == "Thai")
                and (
                    uroman.dict_str[("syllable-info", prev_char)]
                    == "written-pre-consonant-spoken-post-consonant"
                )
            ...
            if (first_char == "\u0E2D") and (end - start == 1):
                ...
                prev_rom = self.find_rom_edge_path_backwards(
                    0, start, 1, return_str=True
                )
        ...
        if (
            next_char
            and (next_char == "\u0300")
            and (uroman.chr_script_name(last_char) == "Coptic")
            and (
                not self.simple_top_romanization_candidate_for_span(orig_start, end + 1)
            )
        ):
            rom = "e" + rom
            end = end + 1
        ...
        y_rom = None
        if (
            (next_char and next_char in "ゃゅょャュョ")
    ...
    def pick_tibetan_vowel_edge(self, **args) -> None:
        ...
        for start in range(self.max_vertex):
            c = s[start]
            if (
                uroman.chr_script_name(c) == "Tibetan"
            ) and self.char_is_letter_or_vowel_sign(c):
        ...
        for tibetan_letter_positions in tibetan_syllable:
            ...
            for i in tibetan_letter_positions:
                ...
                orig_txt += c
                rom = first_non_none(
                    self.simple_top_romanization_candidate_for_span(i, i + 1), "?"
                )
                self.props[("edge-vowel", i)] = None
                if self.char_is_vowel_sign(c) or (
                    rom and regex.match(r"[aeiou]+$", rom)
                ):
                    vowel_pos = i
                    ...
                    c == "\u0F60"
                ):  # Tibetan letter -a, romanized as an apostrophe ("'")
            ...
            if vowel_pos is not None:
                ...
                best_cost, best_vowel_pos, best_pre, best_post = (
                    math.inf,
                    None,
                    None,
                    None,
                ...
                for i in tibetan_letter_positions:
                    ...
                    pre, post = "".join(roms[: rel_pos + 1]), "".join(
                        roms[rel_pos + 1 :]
                    )
                    if self.props.get(("edge-vowel", i)) is False:
                        ...
                        good_suffix = regex.match(
                            r"(?:|[bcdfghjklmnpqrstvwxz]|bh|bs|ch|cs|dd|ddh|"
                            r"dh|dz|dzh|gh|gr|gs|kh|khs|kss|n|nn|nt|ms|ng|ngs|ns|ph|"
                            r"rm|sh|ss|th|ts|tsh|tt|tth|zh|zhs)'?$",
                            post,
                        )
                        # noinspection SpellCheckingInspection
                        good_prefix = regex.match(
                            r"'?(?:.|bd|br|brg|brgy|bs|bsh|bst|bt|bts|by|bz|bzh|"
                            ...
                            r"sk|skr|sky|sl|sm|sn|sny|sp|spy|sr|st|th|ts|tsh)$",
                            pre,
                        )
                        subjoined_suffix = all(
                            [
                                x in subjoined_letter_positions
                ...
                if verbose:
                    best_cost = (
                        best_cost if isinstance(best_cost, int) else round(best_cost, 2)
                    )
                    sys.stderr.write(
                        f'Tib. best cost: "{best_pre}a{best_post}"  o:{orig_txt}  c:{round(best_cost, 2)}'
                        f"   p:{best_vowel_pos} {tibetan_letter_positions}\n"
                    )

    def add_default_abugida_vowel(
        self, rom: str, start: int, end: int, annotation: str = ""
    ) -> str:
        ...
        try:
            ...
            if key in uroman.abugida_cache:
                ...
                vowels_regex2 = "|".join(
                    map(lambda x: x + "+", abugida_default_vowels)
                )  # e.g. 'a+' or 'a+|o+'
                # noinspection SpellCheckingInspection
                if m := regex.match(rf"([cfghkmnqrstxy]?y)({vowels_regex2})-?$", rom):
                    base_rom = m.group(1)
                    base_rom_plus_vowel = base_rom + m.group(2)
                elif m := regex.match(
                    rf"([bcdfghjklmnpqrstvwxyz]+)({vowels_regex1})-?$", rom
                ):
                    base_rom = m.group(1)
                    base_rom_plus_vowel = base_rom + m.group(2)
                ...
                if not regex.match(r"[bcdfghjklmnpqrstvwxyz]+$", base_rom) and (
                    not ((script_name == "Tibetan") and (base_rom == "'"))
                ):
            ...
            if (
                next_s_char
                and ((base_rom in "bcdfghklmnpqrstvwz") or (base_rom in ["ng"]))
                and (next_s_char in "យ")
            ):  # Khmer yo
                return base_rom
            ...
            if self.char_is_subjoined_letter(next_s_char):
                return base_rom
            if (
                self.uroman.char_is_nonspacing_mark(next_s_char)
            ...
            if self.is_at_end_of_word(end):
                if (script_name in ("Devanagari",)) and (
                    self.lcode not in ("san",)
                ):  # Sanskrit
                    return rom
    ...
    def cand_is_valid(self, rom_rule: RomRule, start: int, end: int, rom: str) -> bool:
        ...
        if rom_rule["dont-use-at-start-of-word"] and self.is_at_start_of_word(start):
            return False
        if rom_rule["use-only-at-start-of-word"] and not self.is_at_start_of_word(
            start
        ):
            return False
        ...
        if rom_rule["use-only-at-end-of-word"] and not self.is_at_end_of_word(end):
            return False
        if rom_rule["use-only-for-whole-word"] and not (
            self.is_at_start_of_word(start) and self.is_at_end_of_word(end)
        ):
            return False
    ...
    def simple_top_romanization_candidate_for_span(
        self, start, end, simple_search: bool = False
    ) -> str | None:
        ...
        for rom_rule in self.uroman.rom_rules[self.s[start:end]]:
            if self.cand_is_valid(rom_rule, start, end, rom_rule["t"]):
                ...
                if best_n_restr is None or (n_restr > best_n_restr):
                    best_cand, best_n_restr, best_rom_rule = (
                        rom_rule["t"],
                        n_restr,
                        rom_rule,
                    )
    ...
    def decomp_rom(self, char_position: int) -> str | None:
        ...
        if ud_decomp_s := ud.decomposition(char):
            ...
            if (
                format_comps
                and (
                    format_comps[0] not in ("<super>", "<sub>", "<noBreak>", "<compat>")
                )
                and (not other_comps)
                and decomp_s
            ):
            ...
            if rom and ud.numeric(char, None):
                rom = rom.replace("⁄", "/")
                if char_position >= 1 and ud.numeric(
                    full_string[char_position - 1], None
                ):
                    rom = " " + rom
                if (char_position + 1 < len(full_string)) and ud.numeric(
                    full_string[char_position + 1], None
                ):
                    rom += " "
        return rom
        ...
        for start in range(self.max_vertex):
            for end in range(start + 1, self.max_vertex + 1):
                if not self.uroman.dict_bool[("s-prefix", self.s[start:end])]:
                    break
                if (
                    rom := self.simple_top_romanization_candidate_for_span(start, end)
                ) is not None:
                    ...
                    new_rom = self.add_default_abugida_vowel(
                        rom, start, end, annotation=edge_annotation
                    )
                    ...
                    rom, start2, end2, exp_edge_annotation = (
                        self.expand_rom_with_special_chars(
                            rom,
                            start,
                            end,
    ...
    def add_numbers(self, uroman, **args):
            ...
            if self.edge_is_digit(edge) and edge.active:  # and (edge.value != 0):
                ...
                while True:
                    ...
                    if self.edge_is_digit(right_edge):
                        ...
                        (prev_edge.end < len(s))
                        and (s[prev_edge.end] == ".")
                        and (n_decimal_points == 0)
                        and (
                            right_edge2 := self.best_right_neighbor_edge(
                                prev_edge.end + 1
                            )
                        )
                ...
                if len(sub_edges) >= 2:
                    new_value = (
                        float(new_value_s) if "." in new_value_s else int(new_value_s)
                    )
                    new_edge = NumEdge(
                        sub_edges[0].start,
                        sub_edges[-1].end,
        ...
        for edge in num_edges:
            if (
                ...
                right_edge = self.best_right_neighbor_edge(
                    edge.end, skip_num_edge=False
                )
                if (
                    right_edge
                    ...
                    new_edge = NumEdge(
                        edge.start, right_edge.end, str(new_value), uroman, active=True
                    )
                    new_edge.update(
                        value=new_value,
                        num_base=right_edge.num_base,
                        e_type="G1",
                        orig_txt=edge.orig_txt + right_edge.orig_txt,
                        script=right_edge.script,
                    )
                    self.add_edge(new_edge)
                    num_edges = self.update_edge_list(
                        num_edges, new_edge, [edge, right_edge]
                    )
                    if verbose:
                        print(new_edge.type, new_edge)
        ...
        for edge in num_edges:
            if (
                ...
                while (
                    ...
                    and (
                        right_edge := self.best_right_neighbor_edge(
                            prev_edge.end, skip_num_edge=False
                        )
                    )
                    and isinstance(right_edge, NumEdge)
                    and right_edge.active
                    ...
                    and (not right_edge.is_large_power)
                    and (
                        self.is_gap_null_edge(prev_non_edge)
                        or (
                            (prev_non_edge.num_base > right_edge.value)
        ...
        for edge in num_edges:
            ...
            for fraction_connector in self.uroman.fraction_connectors:
                ...
                if not hasattr(right_edge, "value") or right_edge.value is None:
                    continue
                if edge.value == 100:
                    if (
                        isinstance(right_edge.value, int)
                        or isinstance(right_edge.value, float)
                    ) and (edge.value >= 0):
                        new_edge = Edge(
                            edge.start,
                            right_edge.end,
        ...
        for edge in num_edges:
            if (
                ...
                and (edge.value is not None)
                and (
                    ((edge.value > 1000) and (edge.start + 1 == edge.end))
        ...
        if (
            verbose
        ):  # or (num_edges and any([e.type in ['G1', 'G2', 'G3', 'G4'] for e in num_edges])):
            if num_edges:
                print("actives:")
        ...
        for start in range(len(s)):
            start_char = s[start]
            if (best_edge := self.best_edge_in_span(start, start + 1)) and isinstance(
                best_edge, NumEdge
            ):
                continue
    ...
    def add_rom_fall_back_singles(self, **_args):
        ...
        for start in range(self.max_vertex):
            ...
            if not self.lattice[(start, end)]:
                ...
                elif self.uroman.char_is_format_char(
                    rom
                ):  # e.g. zero-width non-joiner, zero-width joiner
                ...
                #     rom, edge_annotation = ' ', 'Zs'
                    ...
                    rom2 := self.simple_top_romanization_candidate_for_span(start, end)
                ) is not None:
                    rom = rom2
                    if regex.match(r"\+(m|ng|n|h|r)", rom):
                        rom = rom[1:]
    ...
    def best_edge_in_span(
        self, start: int, end: int, skip_num_edge: bool = False
    ) -> Edge | None:
    ...
    def best_right_neighbor_edge(
        self, start: int, skip_num_edge: bool = False
    ) -> Edge | None:
        for end in sorted(list(self.lattice[(start, "right")]), reverse=True):
            if best_edge := self.best_edge_in_span(
                start, end, skip_num_edge=skip_num_edge
            ):
                return best_edge
        return None

    def best_left_neighbor_edge(
        self, end: int, skip_num_edge: bool = False
    ) -> Edge | None:
    ...
    def best_rom_edge_path(
        ...
        start2 = start
        while start2 < end:
            if best_edge := self.best_right_neighbor_edge(
                start2, skip_num_edge=skip_num_edge
            ):
    ...
    def find_rom_edge_path_backwards(
        self,
        start: int,
        end: int,
        min_char: int | None = None,
        return_str: bool = False,
        skip_num_edge: bool = False,
    ) -> List[Edge] | str:
        ...
        while start < end2:
            old_end2 = end2
            if new_edge := self.best_left_neighbor_edge(
                end2, skip_num_edge=skip_num_edge
            ):
...
def main():
    ...
    parser.add_argument(
        "--data_dir", type=Path, default=None, help="uroman resource dir"
    )
    ...
    parser.add_argument(
        ...
        "--rom_format",
        type=RomFormat,
        default=RomFormat.STR,
        choices=list(RomFormat),
        help="Output format of romanization. 'edges' provides offsets",
    )
    # The remaining arguments are mostly for development and test
    parser.add_argument(
        "--max_lines", type=int, default=None, help="limit uroman to first n lines"
    )
    parser.add_argument(
        "--load_log", action="count", default=0, help="report load stats (boolean)"
    )
    parser.add_argument(
        "--test", action="count", default=0, help="perform/display a few tests"
    )
    parser.add_argument(
        "-d",
        "--decode_unicode",
    ...
    parser.add_argument(
        ...
        "--cache_size",
        type=int,
        default=DEFAULT_ROM_MAX_CACHE_SIZE,
        help="for speed",
    )
    parser.add_argument(
        "--silent", action="count", default=0, help="suppress ... progress"
    )
    parser.add_argument(
        "-a", "--ablation", type=str, default="", help="for development mode: nocap"
    )
    parser.add_argument(
        "--stats", action="count", default=0, help="for development mode: numbers"
    )
    parser.add_argument(
        "--ignore_args", action="count", default=0, help="for usage illustration only"
    )
    parser.add_argument(
        PROFILE_FLAG,
        ...
        help="(optional output for performance analysis)",
    )
    parser.add_argument(
        "--version",
        action="version",
    ...
    args_dict = {
        ...
        "rebuild_ud_props": args.rebuild_ud_props,
    ...
    if args.ignore_args:
        ...
        print(
            s4, Edge.json_str(uroman.romanize_string(s4, rom_format=RomFormat.LATTICE))
        )
        ...
        romanize_file_p = (
            args.input_filename
            or args.output_filename
            or not (
                args.direct_input
                or args.test
                or args.ignore_args
                or args.rebuild_ud_props
                or args.rebuild_num_props
            )
        )
